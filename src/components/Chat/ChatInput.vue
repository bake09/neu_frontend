<template>
  <div class="flex column text-black">
    ...
    >
    <q-card flat class="reply-box bg-white" :style="{ height: '80px', width: `${inputWidth}px` }" v-if="chatStore.showReplayBox">
      <q-card-section class="q-ma-sm bg-grey-2 relative-position flex" style="height: 44px; border-left: 5px solid green;">
        ...
        <q-btn style="position: absolute; right:0; top:0;" color="grey-6" flat padding="xs" icon="close" @click="chatStore.showReplayBox = false"/>
      </q-card-section>
    </q-card>
    </transition-group>
    ...
    <div class="row">
      <div class="flex col input-wrapper q-px-sm q-mr-sm bg-white" ref="inputWrapper" >
        <q-btn dense round flat icon="mood" class="text-grey self-end q-mb-xs q-mr-sm" @click="chatStore.showReplayBox = !chatStore.showReplayBox"/>
        <q-input 
          autogrow 
          dense 
          borderless 
...
onMounted(() => {
  chatStore.inputWrapper = inputWrapper.value.offsetWidth
  inputWidth.value = chatStore.inputWrapper
  // console.log('inputWrapper :>> ', inputWrapper)
  // console.log('chatStore.inputWrapper :>> ', chatStore.inputWrapper)
...
.slide-enter-active {
  transition: transform 0.3s ease, opacity 0.3s ease 0.15s, height 0.3s ease; /* Delay opacity for a smoother effect */
}

.slide-enter-from {
  transform: translateY(100%);
  opacity: 0;
  height: 40; /* Start height at 0 */
}
...
.slide-leave-from {
  transform: translateY(100%);
  opacity: 1;
  height: 80; /* Start from the target height */
}

.slide-leave-to {
  transform: translateY(0);
  opacity: 0;
  height: 0; /* End height at 0 */
}
</style>
